<?php
// product_in_a_grid.php
// Objective: find the largest product of 4 adjacent numbers in the below 20x20
// Constraints:
//     Checks only 4 adjacent numbers, no more or less
//     Checks all 4 possible directions (vertical, horizontal, positive diagonal, negative diagonal)


// 20x20 2 dimensional array of numbers (0-99, inclusive)
// usage $grid[$row][$col], where 0 ≤ $row ≤ 19 and 0 ≤ $col ≤ 19
...
// declare and initialize the variable to hold the largest variable
$largestProduct = 0;



// YOUR CODE GOES HERE
// You will most likely need 4 for loops
for ($row=0; $row <20; $row++) { // for each row in grid
	for ($col=0; $col <20; $col++) { // for each column in the row
		// echo $grid[$row][$col]." ";  // print value
	} 
	// echo "<br>"; // print separator
}
...
for ($row=0; $row <20; $row++) { // for each row in grid
	...
	for ($col=0; $col < count($thisRow)-3; $col++) { 
		...
		$product = $thisRow[$col]*$thisRow[$col+1]*$thisRow[$col+2]*$thisRow[$col+3];
		//echo $product;
...
for ($row=0; $row <20-3; $row++) { 
	for ($col=0; $col < count($grid[$row]); $col++) { 
		echo "(".$grid[$row][$col].", ".$grid[$row + 1][$col].", ".$grid[$row + 2][$col].", ".$grid[$row + 3][$col].")\n"; 
...
for ($row=0; $row <20-3; $row++) { 
	for ($col=0; $col < count($grid[$row])-3; $col++) { 
		echo "(".$grid[$row][$col].", ".$grid[$row + 1][$col+1].", ".$grid[$row + 2][$col+2].", ".$grid[$row + 3][$col+3].")\n"; 
...
for ($row=0; $row <20-3; $row++) { 
	for ($col=0; $col < count($grid[$row])-3; $col++) { 
		echo "(".$grid[$row+3][$col+0].", ".$grid[$row + 2][$col+1].", ".$grid[$row + 1][$col+2].", ".$grid[$row + 0][$col+3].")\n"; 
		$product = $grid[$row+3][$col+0]*$grid[$row + 2][$col+1]*$grid[$row + 1][$col+2]*$grid[$row + 0][$col+3];
		//echo $product;
		if ($product > $largestProduct) {
			$largestProduct = $product;
		}
...
/*
$row = 0
$thisRow=$grid[0]= Array( 8,  2, 22, 97, 38, 15,  0, 40,  0, 75,  4,  5,  7, 78, 52, 12, 50, 77, 91,  8),
$thisRow[0] = 8, $thisRow[1] = 2, etc
using the for loop inside of the other one lets us iterate through each column
