...
foreach ($emails as $email) {
	$email = trim($email); // remvoe whitespace from cURL
	...
	if (!preg_match("/.[a-zA-Z]{2,}$/", $email)) { //Contains nothing but letters in the domain (the part after the . at the end)
		$numFailings++;
	}

	if (!preg_match("/.[a-zA-Z]{2,}/", $email)) { //Domain is at least 2 characters long.
		$numFailings++;
	}

	if (!preg_match("/[A-Z._%+-]/", $email)) { //Only contains letters, numbers, and ._%+-
		$numFailings++;
	}
	...
	if ($numFailings > 1) { // check if more than 1 failure occured
		...
		if (!preg_match("/.[a-zA-Z]{2,}/", $email)) { // domain needs to be 2 characters long
			echo "Domain needs to be atleast 2 characters long";
		}
		if (!preg_match("/.[a-zA-Z]{2,}$/", $email)) { //domain only contain letters
			echo "Has to contain only letters in the domain";

		}
		if (!preg_match("/[A-Z._%+-]/", $email)) {
